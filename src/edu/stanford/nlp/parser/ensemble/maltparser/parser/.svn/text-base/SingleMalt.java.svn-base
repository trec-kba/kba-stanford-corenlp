...
import edu.stanford.nlp.parser.ensemble.maltparser.core.config.ConfigurationException;
...
public class SingleMalt implements DependencyParserConfig {
	...
	public void initialize(int containerIndex, DataFormatInstance dataFormatInstance, ConfigurationDir configDir, int mode) throws MaltChainedException {
		...
		registry.put(edu.stanford.nlp.parser.ensemble.maltparser.core.symbol.SymbolTableHandler.class, getSymbolTables());
		registry.put(edu.stanford.nlp.parser.ensemble.maltparser.core.io.dataformat.DataFormatInstance.class, dataFormatInstance);
...
//		}
		...
		if (mode == LEARN) {
			...
			if (configLogger.isDebugEnabled()) {
				configLogger.debug("Parsing time: " +new Formatter().format("%02d:%02d:%02d", elapsed/3600000, elapsed%3600000/60000, elapsed%60000/1000)+" ("+elapsed+" ms)\n");
			}
		}
