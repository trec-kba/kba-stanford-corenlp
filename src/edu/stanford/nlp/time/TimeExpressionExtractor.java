...
public class TimeExpressionExtractor {
  ...
  {
    timeIndex.clear();
    timeIndex.addTemporal(SUTime.TIME_REF);
  }
  ...
  public List<CoreMap> extractTimeExpressionCoreMaps(CoreMap annotation, String docDate) 
  {
    ...
    return toCoreMaps(annotation, timeExpressions);
  }

  private List<CoreMap> toCoreMaps(CoreMap annotation, List<TimeExpression> timeExpressions)
  {
    ...
    for (TimeExpression te:timeExpressions) {
      ...
      /*    Integer offset = annotation.get(CoreAnnotations.CharacterOffsetBeginAnnotation.class);
        ...
        Timex timex = Timex.fromMap(text, timexAttributes);
  ...
  public List<TimeExpression> extractTimeExpressions(CoreMap annotation, String docDateStr)
  {
    ...
    List<TimeExpression> timeExpressions2 = new ArrayList<TimeExpression>();
    for (TimeExpression te : timeExpressions) {
    	if (te != null && te.tokenOffsets != null &&
    			te.tokenOffsets.first() != null &&
    			te.tokenOffsets.second() != null) 
    		timeExpressions2.add(te);
    }
    timeExpressions = removeNested(timeExpressions2);
